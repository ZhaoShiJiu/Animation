"""
graphs/paper_graph.py — PDF 论文 → 动画 单图流程。

路由: analyze_paper → generate_segments ⇄ validate_segments → assemble → postprocess → END

替换 /paper/generate 路由。
"""
from langgraph.graph import StateGraph, END

from backend.graph.state import AnimationState
from backend.graph.nodes.plan import analyze_paper
...
def build_paper_graph():
    ...
    graph.add_edge("analyze_paper", "generate_segments")
